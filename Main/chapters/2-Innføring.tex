...
\verb|cat| kan også brukes til å \textit{pipe} output fra en kommando inn i en annen kommando. Symbolene for piping kan være \text{\textbar} eller \verb|>| ut fra formålet. Dersom man ønsker at innholdet i \verb|main.c| skal mates inn i kommandoen \verb|less|, som er et verktøy for å lese innholdet i tekstfiler, kan man \textit{pipe} ved bruk av kodeordet \text{\textbar} på følgende måte:

\begin{lstlisting}[mathescape=true]
student@Ubuntu:$\smallsim$$\verb|$|$ cat main.c | less
\end{lstlisting}

Dersom man føler at innholdet i \verb|main.c| hører bedre hjemme i \verb|silly_greeting.c|, kan man kopiere innholdet i \verb|main.c| til \verb|silly_greeting.c| gjennom piping med kodeordet \verb|>| på følgende måte:

\begin{lstlisting}[mathescape=true]
student@Ubuntu:$\smallsim$$\verb|$|$ cat main.c > silly_greeting.c
